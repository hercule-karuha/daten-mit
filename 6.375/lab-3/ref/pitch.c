...
#define NUM_TAPS 8
double coeffs[9] = {-0.0124, 0.0,     -0.0133, 0.0,    0.8181,
                    0.0,     -0.0133, 0.0,     -0.0124};

// Get the integer part from a double.
// This is meant to correspond to fxptGetInt in bluespec.
short getInt(double x) { return x < 0 ? (short)(x - 1) : (short)x; }
...
complex double cmplxmp(double mag, double phs) { return mag * cexp(I * phs); }
...
void pitchadjust(complex double *in, complex double *out) {
  ...
  int i;
  printf("----------a new vector input----------\n");
  for (i = 0; i < N; i++) {
    double phase = carg(in[i]);
    double mag = cabs(in[i]);
    printf("ti1[%d] = cmplxmp(%lf, tophase(%lf));\n", i, mag, phase);
  ...
  printf("----------a new vector input end----------\n");
  printf("----------a new vector output----------\n");
  for (i = 0; i < N; i++) {
    printf("ti1[%d] = cmplxmp(%lf, tophase(%lf));\n", i, cabs(out[i]),
           carg(out[i]));
  }
  printf("----------a new vector output end----------\n");
}
...
int main(int argc, char *argv[]) {
  ...
  fftw_plan forward =
      fftw_plan_dft_1d(N, infft, outfft, FFTW_FORWARD, FFTW_ESTIMATE);
  fftw_plan reverse =
      fftw_plan_dft_1d(N, infft, outfft, FFTW_BACKWARD, FFTW_ESTIMATE);
